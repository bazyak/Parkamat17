...
class HideWizardImageFragment : Fragment() {
    ...
    private val viewModel: HideWizardImageViewModel by lazy { (requireActivity() as AndroidScopeComponent).scope.get() }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.liveDataShowImageTooSmall.observe(viewLifecycleOwner) {
            if (it) {
        ...
        viewModel.liveDataShowImageTooLarge.observe(viewLifecycleOwner) {
            if (it) {
        ...
        childFragmentManager.beginTransaction().replace(R.id.containerFilePickerImage, filePickerFragment).commit()
    }
    ...
    private fun showFragment(fragment: Fragment?) {
        if (fragment != null) {
            childFragmentManager.beginTransaction().replace(R.id.containerMessage, fragment).commit()
        } else {
            childFragmentManager.findFragmentById(R.id.containerMessage)
                ?.let { shownFragment -> childFragmentManager.beginTransaction().remove(shownFragment).commit() }
        }
    }
