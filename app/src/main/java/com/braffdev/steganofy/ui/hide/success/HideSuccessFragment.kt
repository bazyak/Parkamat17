...
class HideSuccessFragment : Fragment() {
    ...
    private val viewModel: HideSuccessViewModel by lazy { (requireActivity() as AndroidScopeComponent).scope.get() }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.liveDataStatisticsHideTime.observe(viewLifecycleOwner) { binding.textViewStatisticsHideTime.text = it }
        ...
        viewModel.liveEventFinish.observe(viewLifecycleOwner) { requireActivity().finish() }
        ...
        childFragmentManager.beginTransaction().replace(R.id.containerImageInfo, messageFragment).commit()
    }
